from arcface.arcface.api import featurize,compare
import pymysql
import ffmpeg
import cv2
...
mll_feature = np.load('mllfeature.npy')
...
while 1:
    ...
    if len(res)>0:
        ...
        for i in range(n):
            person = crop_image[i,:,:,:]
            feature = featurize(person)
            # print(feature,feature.shape)
            score = compare(feature,mll_feature)
            end_time = time.time()
            if score>0.7:
                print('你好孟令龙！,打卡成功','打卡时间:',nowTime,' runtime: ',round(float(end_time-start),4),'s')
    else:
        frame = frame
